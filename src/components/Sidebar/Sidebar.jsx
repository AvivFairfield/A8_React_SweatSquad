...
export const Sidebar = () => {
    ...
    return (
        <>
            <button
                className={`toggle-sidebar fixed z-100 left-5 top-5 cursor-pointer bg-gray-500 px-1 text-xl sidebar ${theme}`}
                onClick={() => setOpen(!open)}
            >
                <FontAwesomeIcon icon={faBars} />
            </button>
            <div
                className={`fixed w-64 top-0 bottom-0 bg-opacity-95 bg-neutral-800 transition-left duration-300 ease-in-out z-50 overflow-y-auto sidebartext ${theme} ${
                    open ? "left-0" : "-left-64"
                }`}
            >
